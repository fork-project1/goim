...
func (r *Ring) init(num uint64) {
	...
	// 将数量设置为2的幂原因：使用【镜像指示位】方法判断环形缓存是否满。如果缓冲区长度是2的幂，则本方法可以省略镜像指示位。
	// 按位与的运算效率要比求余运算高的多，在linux内核中将缓冲区长度扩展为2的幂长度随处可见，都是为了用按位与操作代替求余操作。
	// 缓冲区的长度是2的幂的情况下，如果读写指针的值相等，则缓冲区为空；如果读写指针相差n，则缓冲区为满。
	if num&(num-1) != 0 {
		for num&(num-1) != 0 {
			num &= num - 1
		}
